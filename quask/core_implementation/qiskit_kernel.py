...
class QiskitKernel(Kernel):
    ...
    def phi(self, x) -> float:
        if self.type == KernelType.OBSERVABLE:
            ...
            raise ValueError("phi not available for fidelity kernels or swap test")
    ...
    def to_qiskit_circuit(self) -> QuantumCircuit:
        ...
        # perform a fake operation in order to force the quantum circuit to use all the parameters
        qc.rx(0.00000000001 * math.prod(params), 0)

        for i in range(self.ansatz.n_operations):
            op = self.ansatz.operation_list[i]
            if op.feature in range(self.ansatz.n_features): 
                feature = params[op.feature]
            else: 
                feature = 1.0
